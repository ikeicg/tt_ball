...
function matchSocketHandle(io: Server): void {
  ...
  matchIO.on("connection", (socket: Socket) => {
      ...
      ({ name, matchId }: { name: string; matchId: string }) => {
        ...
        if (!match.playerList.has(playerToken)) {
          ...
          if (match.matchStarted) {
            matchIO.to(matchId).except(socket.id).emit("share_state", {});
            return;
          }
          ...
          if (match.playerList.size === 2) {
            ...
            matchIO.to(matchId).emit("start_match", matchState); // send fresh game/match state
          }
        }
      }
    ); // complete
    ...
    socket.on("sharing_state", (matchId: string, message: object) => {
      matchIO.to(matchId).except(socket.id).emit("sharing_state", { message });
    });
    ...
    socket.on("attack_attempt", (matchData) => {
      ...
      if (state.role !== 0) {
        ...
        matchIO.to(matchId).emit("upgrade_state", matchData);
        return;
      }
      ...
      matchIO
        .to(matchId)
        .emit("upgrade_state", { ...matchData, state: newState });
    });
    ...
    socket.on("initiate_scoring", (matchId) => {
      matchIO.to(matchId).emit("send_moves", {});
    });
    ...
    socket.on("game_moves", ({ matchId, moves }) => {
      matchIO.to(matchId).except(socket.id).emit("sending_moves", { moves });
    });
    ...
    socket.on(
      ...
      ({
        ...
        if (fairnessCheck) {
          ...
          if (stage >= maxStage) {
            matchIO.to(matchId).emit("end_game", {});
            return;
          }

          stage++;
          let newState = { ...matchData.state, stage };
          matchIO.to(matchId).emit("upgrade_state", {
            ...matchData,
            state: newState,
          });
        }
